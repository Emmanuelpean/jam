...
class TestGet:
    ...
    def test_get_all(
        ...
        compare(aggregators, test_aggregator)
    ...
    def test_get_one(
        ...
        response = authorized_client1.get(f"/aggregators/{test_aggregator[0].id}")
        assert response.status_code == 200
        compare(schemas.Aggregator(**response.json()), test_aggregator[0])
    ...
    def test_get_one_unauthorised_user(
        ...
        response = client.get(f"/aggregators/{test_aggregator[0].id}")
        assert response.status_code == 401
...
class TestDelete:
    ...
    def test_success(
        ...
        response = authorized_client1.delete(f"/aggregators/{test_aggregator[0].id}")
    ...
    def test_unauthorized_user(
        ...
        response = client.delete(f"/aggregators/{test_aggregator[0].id}")
        assert response.status_code == 401
...
class TestPut:
    ...
    def test_empty(
        ...
        response = authorized_client1.put(f"/aggregators/{test_aggregator[0].id}", json={})
