...
class TestMaintenanceModeAuthEndpoints(MaintenanceTestBase):
    ...
    def test_login_blocked_during_maintenance(
    ...
    def test_email_verification_blocked_during_maintenance(
        self, test_unverified_user: FixtureUser, test_admin_user: FixtureUser
    ) -> None:
        """Email verification is blocked when maintenance is active."""

        plain_token: str = test_unverified_user.create_token(TokenType.EMAIL_VERIFICATION)[0]
        past_time = self._get_past_timestamp(minutes=5)
        self._set_maintenance_scheduled_at(past_time, test_admin_user)
