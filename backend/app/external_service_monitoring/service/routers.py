...
from app.external_service_monitoring.service.sync import SERVICE_NAME, service_monitoring_runner
from app.service_runner import routers

external_service_monitoring_service_router = APIRouter(
    prefix="/external-service-monitoring-service",
    tags=["external-service-monitoring-service"],
)


@external_service_monitoring_service_router.post("/start")
def start(
    current_user: models.User = Depends(get_current_user),
) -> dict:
    """Start the service-monitoring runner. Admin only."""

    return routers.start_scraper(service_monitoring_runner, current_user, 24)


@external_service_monitoring_service_router.post("/stop")
def stop(
    current_user: models.User = Depends(get_current_user),
) -> dict:
    """Stop the service-monitoring runner. Admin only."""

    return routers.stop_scraper(service_monitoring_runner, current_user)


@external_service_monitoring_service_router.get("/status")
def status(
    current_user: models.User = Depends(get_current_user),
) -> dict:
    """Get the current runner status. Admin only."""

    return routers.scraper_status(service_monitoring_runner, current_user)


@external_service_monitoring_service_router.get("/logs")
def get_logs(
...
external_service_monitoring_service_log_router = APIRouter(
    prefix="/external-service-monitoring-service-logs",
    tags=["external-service-monitoring-service-logs"],
)


@external_service_monitoring_service_log_router.get("/", response_model=list[ServiceMonitoringServiceLogOut])
...
@external_service_monitoring_service_log_router.get("/latest", response_model=ServiceMonitoringServiceLogOut)
def get_latest(
    current_user: models.User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
