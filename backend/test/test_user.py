...
def test_get_current_user_profile_success(authorized_client1, test_user1):
    ...
    assert user_profile.email == test_user1["email"]
    assert user_profile.id == test_user1["id"]
...
def test_update_current_user_profile_email_success(authorized_client1, test_user1):
    """Test successfully updating user email."""
    update_data = {"email": "newemail@example.com"}
    response = authorized_client1.put("/users/me", json=update_data)

    assert response.status_code == 200
    user_data = response.json()
    assert user_data["email"] == "newemail@example.com"
    assert user_data["id"] == test_user1["id"]


def test_update_current_user_profile_theme_success(authorized_client1, test_user1):
...
def test_update_current_user_profile_password_success(authorized_client1, test_user1, session):
    ...
    user_in_db = session.query(models.User).filter(models.User.id == test_user1["id"]).first()

    # Password should be hashed, not plain text
    assert user_in_db.password != "new_secure_password123"
    assert user_in_db.password != test_user1["password"]


def test_update_current_user_profile_multiple_fields(authorized_client1, test_user1):
...
def test_update_current_user_profile_email_already_exists(authorized_client1, test_user2):
    """Test updating email to one that already exists."""
    # Try to update to test_user2's email
...
def test_update_current_user_profile_same_email(authorized_client1, test_user1):
    """Test updating email to the same email (should succeed)."""
    update_data = {"email": test_user1["email"]}
    response = authorized_client1.put("/users/me", json=update_data)

    assert response.status_code == 200
    user_data = response.json()
    assert user_data["email"] == test_user1["email"]


def test_update_current_user_profile_invalid_theme(authorized_client1):
...
def test_update_current_user_profile_empty_body(authorized_client1):
    """Test updating with empty request body."""
    response = authorized_client1.put("/users/me", json={})

    # Should succeed as partial updates are allowed
    assert response.status_code == 200


def test_update_current_user_profile_unauthorized(client):
...
def test_update_current_user_profile_partial_update(authorized_client1, test_user1):
    """Test that only provided fields are updated."""
    # Get original user data
    original_response = authorized_client1.get("/users/me")
...
def test_update_current_user_profile_invalid_email_format(authorized_client1, invalid_email):
...
def test_update_current_user_profile_null_fields(authorized_client1):
...
def test_update_current_user_profile_theme_case_sensitivity(authorized_client1):
...
def test_get_and_update_user_profile_integration(authorized_client1, test_user1):
    """Integration test: get profile, update it, then verify changes."""
    # Get original profile
    get_response = authorized_client1.get("/users/me")
    assert get_response.status_code == 200
    original_data = get_response.json()

    # Update profile
    update_data = {"email": "integration_test@example.com", "theme": "forest-berry"}
    update_response = authorized_client1.put("/users/me", json=update_data)
    assert update_response.status_code == 200

    # Get updated profile
    get_updated_response = authorized_client1.get("/users/me")
