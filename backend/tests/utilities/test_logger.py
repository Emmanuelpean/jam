...
class TestGetLastLogLine:
    ...
    def test_decodes_invalid_utf8_with_replacement(self, tmp_path) -> None:
        log_file = tmp_path / "badutf8.log"
...
class TestReadLogTail:
    ...
    def test_returns_empty_when_file_does_not_exist(self, tmp_path) -> None:
        with patch("app.utilities.logger.settings") as mock_settings:
            mock_settings.log_directory = str(tmp_path)
            result = AppLogger.read_logger("missing").read_log_tail(10)
        assert result == {"lines": [], "total_lines": 0}

    def test_small_file_returns_last_n_lines(self, tmp_path) -> None:
    ...
    def test_small_file_returns_all_when_requested_exceeds_total(self, tmp_path) -> None:
        log_file = tmp_path / "small_all.log"
        log_file.write_text("line1\nline2\n", encoding="utf-8")
        with patch("app.utilities.logger.settings") as mock_settings:
            mock_settings.log_directory = str(tmp_path)
    ...
    def test_large_file_reads_from_end(self, tmp_path) -> None:
    ...
    def test_large_file_requesting_more_than_total_returns_all(self, tmp_path) -> None:
        log_file = tmp_path / "big_all.log"
    ...
    def test_large_file_decodes_invalid_utf8(self, tmp_path) -> None:
        log_file = tmp_path / "big_badutf8.log"
    ...
    def test_returns_error_entry_on_read_failure(self, tmp_path) -> None:
        log_file = tmp_path / "boom.log"
...
class TestGetLogger:
    ...
    def test_creates_file_and_console_handlers(self, log_dir) -> None:
    ...
    def test_console_output_false_skips_console_handler(self, log_dir) -> None:
    ...
    def test_returns_cached_instance(self, log_dir) -> None:
        first = AppLogger.get_logger("ut_cached")
        second = AppLogger.get_logger("ut_cached")
        assert first is second

    def test_reuses_existing_handlers_when_cache_cleared(self, log_dir) -> None:
    ...
    def test_custom_log_file_name(self, log_dir) -> None:
...
class TestReadLogger:
    ...
    def test_returns_instance_without_handlers(self, tmp_path) -> None:
        with patch("app.utilities.logger.settings") as mock_settings:
            mock_settings.log_directory = str(tmp_path)
        ...
        assert not (tmp_path / "ut_readonly.log").exists()
...
class TestCreateServiceLogger:
    ...
    def test_maps_level_string(self, log_dir, level_str, expected) -> None:
        logger = AppLogger.create_service_logger(f"ut_svc_{level_str.lower()}", level_str)
        assert logger.level == expected

    def test_unknown_level_defaults_to_info(self, log_dir) -> None:
        logger = AppLogger.create_service_logger("ut_svc_unknown", "VERBOSE")
        assert logger.level == logging.INFO

    def test_creates_named_log_file(self, log_dir) -> None:
