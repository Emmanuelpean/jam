...
class TestGmailScraper:
    ...
    def test_save_job_ids_to_db(self, session, test_users) -> None:
        ...
        job_records = GmailScraper.save_jobs_to_db(email_record, job_ids, session)
    ...
    def test_save_job_ids_to_db_duplicates(self, session, test_users) -> None:
        ...
        first_records = GmailScraper.save_jobs_to_db(email_record, job_ids, session)
        assert len(first_records) == 2

        # Try to save same job IDs again
        second_records = GmailScraper.save_jobs_to_db(email_record, job_ids, session)
