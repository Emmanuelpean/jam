...
class TestServiceLog:
    ...
    def test_get_service_logs_no_filters(self, admin_client, test_job_rating_service_logs) -> None:
        ...
        data = response.json()
        assert len(data) == len(test_job_rating_service_logs)
        assert data[0]["run_datetime"] >= data[-1]["run_datetime"]

    def test_get_service_logs_with_start_date(
        self, admin_client, test_job_rating_service_logs, test_platform_stats
    ) -> None:
    ...
    def test_get_service_logs_with_limit(
        self, admin_client, test_job_rating_service_logs, test_platform_stats, limit: int
    ) -> None:
    ...
    def test_get_service_logs_non_admin_forbidden(
        self, regular_user_client, test_job_rating_service_logs, test_platform_stats
    ) -> None:
