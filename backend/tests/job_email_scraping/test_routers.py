...
# ----------------------------------------- JOB ALERT EMAILS BY SCRAPED JOB -------------------------------------------
...
class TestScrapedJobsByEmail:
    ...
    def test_not_found_other_users_email(
        self, regular_user_client, test_scraped_jobs, test_job_alert_emails
    ) -> None:
...
class TestJobAlertEmailsPaged:
    ...
    def test_sort_by_subject(self, regular_user_client, test_job_alert_emails) -> None:
        ...
        response = regular_user_client.get(
            self.endpoint, params={"sort_by": "subject", "sort_direction": "asc"}
        )
    ...
    def test_search_by_subject(self, regular_user_client, test_job_alert_emails) -> None:
        ...
        for item in data["items"]:
            assert any(
                "python" in (item.get(field) or "").lower()
                for field in ["subject", "sender", "platform", "alert_name"]
            )
    ...
    def test_search_by_platform(self, regular_user_client, test_job_alert_emails) -> None:
        ...
        for item in data["items"]:
            assert any(
                "indeed" in (item.get(field) or "").lower()
                for field in ["subject", "sender", "platform", "alert_name"]
            )
