...
class TestInit:
    ...
    def test_single_string_job_id_wrapped_in_list(self, mock_apify_cls: MagicMock) -> None:
    ...
    def test_list_job_ids_preserved(self, mock_apify_cls: MagicMock) -> None:
        """List of job_ids is stored as-is."""
        scraper = ConcreteApifyJobScraper(["job1", "job2", "job3"])
        assert scraper.job_ids == ["job1", "job2", "job3"]

    def test_urls_constructed_from_base_url(self, mock_apify_cls: MagicMock) -> None:
...
class TestStartActorRun:
    ...
    def test_starts_actor_with_correct_input(self, mock_apify_cls: MagicMock) -> None:
        ...
        mock_apify_cls.return_value.actor.assert_called_with("test/actor")
        mock_apify_cls.return_value.actor.return_value.start.assert_called_once_with(
            run_input={
                "startUrls": [
...
class TestWaitForData:
    ...
    def test_returns_immediately_on_succeeded(self, mock_sleep: MagicMock, mock_apify_cls: MagicMock) -> None:
        ...
        mock_apify_cls.return_value.run.assert_called_with("run_123")
...
class TestRetrieveData:
    ...
    def test_returns_items_from_dataset(self, mock_apify_cls: MagicMock) -> None:
        """Returns the list of items from the specified Apify dataset."""
        items = [{"title": "Job 1"}, {"title": "Job 2"}]
        mock_apify_cls.return_value.dataset.return_value.list_items.return_value.items = items

        scraper = ConcreteApifyJobScraper("job1")
        result = scraper._retrieve_data("ds_123")

        mock_apify_cls.return_value.dataset.assert_called_once_with("ds_123")
        assert result == items

    def test_raises_when_dataset_empty(self, mock_apify_cls: MagicMock) -> None:
        """Raises Exception when the dataset returns no items."""
        mock_apify_cls.return_value.dataset.return_value.list_items.return_value.items = []
...
class TestProcessJobData:
    ...
    def test_raises_on_base_class(self, mock_apify_cls: MagicMock) -> None:
