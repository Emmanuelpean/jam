...
class TestRunEmailScraperMain:
    ...
    @patch("app.job_email_scraping.run_email_scraper.JobEmailScrapingServiceLogOut")
    @patch("app.job_email_scraping.run_email_scraper.JobEmailScraper")
    def test_default_days(self, mock_scraper_cls, mock_log_out, capsys) -> None:
        """Runs scraper with default days=1 and prints Done."""

        mock_log_out.model_validate.return_value.is_success = True
    ...
    @patch("app.job_email_scraping.run_email_scraper.JobEmailScrapingServiceLogOut")
    @patch("app.job_email_scraping.run_email_scraper.JobEmailScraper")
    def test_custom_days(self, mock_scraper_cls, mock_log_out) -> None:
    ...
    @patch("app.job_email_scraping.run_email_scraper.JobEmailScrapingServiceLogOut")
    @patch("app.job_email_scraping.run_email_scraper.JobEmailScraper")
    def test_short_flag_days(self, mock_scraper_cls, mock_log_out) -> None:
    ...
    @patch("app.job_email_scraping.run_email_scraper.JobEmailScrapingServiceLogOut")
    @patch("app.job_email_scraping.run_email_scraper.JobEmailScraper")
    def test_scraping_error_exits(self, mock_scraper_cls, mock_log_out, capsys) -> None:
        """Failed scraping run prints error message and exits with code 1."""

        mock_log_out.model_validate.return_value.is_success = False
        ...
        assert isinstance(error, SystemExit)
        assert error.code == 1
        assert "scraping run failed" in capsys.readouterr().out
